...
before(async () => {
  ...
  /*
   * The order lives in the database now. It used to be pushed into the
   * browser's localStorage, which is where the app kept its data — seeding
   * that today seeds nothing, and the page renders an order that is not there.
   */
  await clearOperations(DATABASE_URL);
  order = await seedOrder(DATABASE_URL);
});
...
describe("app chrome on the order detail route", needsDatabase, () => {
  test("is server-rendered, before any JavaScript runs", async () => {
    ...
    const { context, page, response } = await openPage(`/orders/${order.orderId}`, {
  ...
  test("is on screen when the URL is opened directly", async () => {
    ...
    assert.equal(chrome.heading, order.orderNo);
  ...
  test("survives navigating in from the orders list", async () => {
    ...
    await page.waitForURL(new RegExp(`/orders/${order.orderId}`));
